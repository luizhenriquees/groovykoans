...
and can become difficult to maintain'''
	...
	void test04_SpecialRegexpOperators() {
		...
		// ------------ START EDITING HERE ----------------------
		def pattern = /(\w+)\s(\w+)/
		def matcher = names =~ pattern

		//Cria duas strings separadas por espaço, essas string podem ser utilizadas na closure, assim como a entrada inicial após aplicação do padrão, podendo ser manipuladas após isso
		...
		// ------------ START EDITING HERE ----------------------
		def visaPattern = /^4[0-9]{12}(?:[0-9]{3})?$/
		isNumberValid = number ==~ visaPattern
		// ------------ STOP EDITING HERE  ----------------------
	...
	void test05_ReplaceAllWithClosure() {
		...
		// ------------ START EDITING HERE ----------------------
		def pattern = /(\w+)/
		def matcher = song =~ pattern
		result = song
		matcher.each { allWords, word ->
			dictionary.each { key, valor ->
				if (word == key) {
					result = result.replace(word, valor)
				}
			}
		}
