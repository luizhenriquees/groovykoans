...
class Koan11 extends GroovyTestCase {
	...
	void test02_MoreOnInsertingData() {
		...
		Sql.withInstance('jdbc:h2:mem:groovyDb2', 'sa', '', 'org.h2.Driver') { db ->
			...
			// ------------ START EDITING HERE ----------------------
			String prefix = 'src/test/groovy/org/groovykoans/koan11/'

			String fileContents = new File("${prefix}cast.txt").text
			def pattern = /(\w+)\s(\w+)/
			def matcher = fileContents =~ pattern
			def params = []
			def insertSql = 'INSERT INTO PERSON (firstname, lastname) VALUES (?, ?)'
			matcher.each { fullName, firstName, lastName ->
				params = [firstName, lastName]
				db.executeInsert insertSql, params
			}
			...
			// ------------ START EDITING HERE ----------------------
			def xml = new XmlSlurper().parse("${prefix}cast2.xml")
			xml.character.@actor.collect {
				matcher = it.text() =~ pattern
				matcher.each { fullName, firstName, lastName ->
					params = [firstName, lastName]
					db.executeInsert insertSql, params
				}
			...
			// ------------ START EDITING HERE ----------------------
			person.add(firstname: 'Luiz Henrique', lastname: 'Fernandes')
			// ------------ STOP EDITING HERE  ----------------------
	...
	void test03_IteratingResults() {
		Sql.withInstance('jdbc:h2:mem:groovyDb3', 'sa', '', 'org.h2.Driver') { db ->
			...
			def lastNameRose
			// ------------ START EDITING HERE ----------------------
			def nameForSearching = 'Rose'
			db.query("SELECT lastname FROM PERSON WHERE firstname LIKE ${nameForSearching}") { resultSet ->
				while (resultSet.next()) {
					def name = resultSet.getString('lastname')
					lastNameRose = "$name"
				}
			}
			// ------------ STOP EDITING HERE  ----------------------
			...
			// ------------ START EDITING HERE ----------------------
			def findVal = '%e%'
			db.eachRow("SELECT COUNT(*) FROM PERSON WHERE lastname LIKE ${findVal}") { row ->
				eCount = "${row[0]}".toInteger()
			}
			// ------------ STOP EDITING HERE  ----------------------
			assert eCount == 2
		}
	...
	void test04_UpdatingTables() {
		Sql.withInstance('jdbc:h2:mem:groovyDb4', 'sa', '', 'org.h2.Driver') { db ->
			...
			// ------------ START EDITING HERE ----------------------
			def findVal = '%a%'
			def updateSql = "UPDATE PERSON SET firstname = 'Alf' WHERE firstname = ?"
			def params = []
			db.eachRow("SELECT firstname FROM PERSON WHERE firstname LIKE ${findVal}") { row ->
				def candidateName = row[0].toString()
