...
class Koan08 extends GroovyTestCase {
	...
	void test01_SwitchStatements() {
		...
		def magicClosure = { input ->
			...
			switch (input) {
				...
				case {input.toString().endsWith('ee')} :
					result = input.toString().substring(0, input.toString().length() -2)
					result += input.toString().substring(input.toString().length() -2, input.toString().length()).replace('ee', 'ey')
					break
			}
			return result
	...
	void test03_MultiAssignment() {
		...
		def generateTwoRandomInts = { int maxInt ->
			...
			return [gerador.nextInt(maxInt), gerador.nextInt(maxInt)]
			// ------------ STOP EDITING HERE  ----------------------
		}
